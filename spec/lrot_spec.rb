  ...
  it "rotates the low 8 bits to the left" do
    100.times do
      ...
      1.upto(7) do |rdist|
        ...
        expect(bnum.lrot8(rdist)).to eq (((bnum & mask) << rdist) | (((bnum & ~mask) & MASK_8) >> (8 - rdist)) | (bnum & ~MASK_8))
      end
    end
  end
  ...
  context "with a Bignum as rotate distance" do
    it "still works the same" do
      100.times do
        ...
        0.upto(20) do |offset|
          rdist = dist + offset
          expect(num.lrot8(rdist)).to eq num.lrot8(rdist % 8)
          expect(bnum.lrot8(rdist)).to eq bnum.lrot8(rdist % 8)
          expect(num.lrot8(rdist) & ~MASK_8).to eq (num & ~MASK_8)
          expect(bnum.lrot8(rdist) & ~MASK_8).to eq (bnum & ~MASK_8)
        end
      end
  ...
  it "rotates the low 16 bits to the left" do
    100.times do
      ...
      1.upto(15) do |rdist|
        ...
        expect(bnum.lrot16(rdist)).to eq (((bnum & mask) << rdist) | (((bnum & ~mask) & MASK_16) >> (16 - rdist)) | (bnum & ~MASK_16))
      end
    end
  end
  ...
  context "with a Bignum as rotate distance" do
    it "still works the same" do
      100.times do
        ...
        0.upto(20) do |offset|
          rdist = dist + offset
          expect(num.lrot16(rdist)).to eq num.lrot16(rdist % 16)
          expect(bnum.lrot16(rdist)).to eq bnum.lrot16(rdist % 16)
          expect(num.lrot16(rdist) & ~MASK_16).to eq (num & ~MASK_16)
          expect(bnum.lrot16(rdist) & ~MASK_16).to eq (bnum & ~MASK_16)
        end
      end
